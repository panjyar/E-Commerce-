...
const ProductCard = ({ product }) => {
      ...
      try {
        ...
        alert(
          error.response?.data?.msg ||
            "Something went wrong. Could not add to cart."
        );
      } finally {
  ...
  const handleToggleWishlist = useCallback(
    async (e) => {
      ...
      try {
        if (isWishlisted) {
          const { data: updatedWishlist } = await api.delete(
          ...
          const { data: updatedWishlist } = await api.post("/wishlist/add", {
            productId: product._id,
          });
          setUser((prevUser) => {
            if (!prevUser) return prevUser;
        ...
        >
          <div className="product-image-container">
            <img
              src={
                product.imageUrl ||
                "https://placehold.co/400x400/e9ecef/495057?text=No+Image"
              }
              alt={product.name}
