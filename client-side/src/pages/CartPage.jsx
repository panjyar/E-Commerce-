...
const CartPage = () => {
  ...
  const handleRemoveFromCart = async (productId) => {
    setLoading(true);
    try {
      const { data: updatedCart } = await api.post('/cart/remove', { productId });
      setUser({ ...user, cart: updatedCart });
    } catch (error) {
  ...
  const totalItems = user?.cart?.reduce((count, item) => count + item.quantity, 0) || 0;

  if (!user || !user.cart || user.cart.length === 0) {
    return (
      <div className="empty-page-message">
        <ShoppingCart size={64} style={{ color: '#6c757d', marginBottom: '1rem' }} />
      ...
      <div style={{ 
        ...
        <div className="cart-items" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {user.cart.map(item => {
            ...
            return (
              <div key={item.product._id} className="cart-item-card" style={{
                ...
                <div style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
                  <Link to={`/product/${item.product._id}`}>
                    <img
                      src={item.product.imageUrl || 'https://placehold.co/120x120/e9ecef/495057?text=No+Image'}
        ...
        <div className="order-summary" style={{
          ...
          <h3 style={{ margin: '0 0 1.5rem', fontSize: '1.4rem' }}>Order Summary</h3>
